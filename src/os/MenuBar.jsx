...
function Menu({ label, brand, items, open, onOpen, onClose }) {
  return (
    <div className={`menubar__item ${brand ? 'menubar__item--brand' : 'menubar__item--menu'}`} role="button" tabIndex={0} aria-haspopup="menu" aria-expanded={open}
      ...
      {open && <div className="menu" role="menu" onClick={e => e.stopPropagation()}>
...
const MenuBar = forwardRef(function MenuBar({ onLock }, ref) {
  ...
  useEffect(() => { if (!open) return; const off = e => { if (!rootRef.current?.contains(e.target)) setOpen(null) }; document.addEventListener('pointerdown', off); return () => document.removeEventListener('pointerdown', off) }, [open])
  ...
  return (
    <header ref={el => { rootRef.current = el; if (typeof ref === 'function') ref(el); else if (ref) ref.current = el }} className="menubar glass chrome" data-glass="menubar" role="menubar">
      <Menu brand items={menus.brand} open={open === 'brand'} onOpen={() => { sounds.click(); setOpen('brand') }} onClose={() => setOpen(null)} />
      <div className="menubar__item menubar__item--app">{app ? app.label : 'Desktop'}</div>
      <Menu label="File" items={menus.file} open={open === 'file'} onOpen={() => setOpen('file')} onClose={() => setOpen(null)} />
      <Menu label="Window" items={menus.window} open={open === 'window'} onOpen={() => setOpen('window')} onClose={() => setOpen(null)} />
