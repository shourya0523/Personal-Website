...
const MenuBar = forwardRef(function MenuBar({ onLock, portalTarget }, ref) {
  ...
  const menus = {
    brand: [{ label: 'About Shourya', onClick: () => os.openApp('about', { source: 'menubar' }) }, '-', { label: 'Settings…', onClick: () => os.openApp('settings', { source: 'menubar' }) }, { label: 'Show desktop', onClick: () => os.setStage(true), disabled: !os.windows.length }, '-', { label: 'Lock screen', onClick: onLock }],
    file: [{ label: 'Open Files', kbd: '⌘F', onClick: () => os.openApp('files', { source: 'menubar' }) }, { label: 'Open Terminal', kbd: '⌘T', onClick: () => os.openApp('terminal', { source: 'menubar' }) }, '-', { label: 'Close window', kbd: '⌘W', disabled: !focused, onClick: () => focused && os.closeWindow(focused.id) }],
