...
- **Liquid glass everywhere.** The menu bar, dock, lock card, menus and every window use [ybouane/liquidglass](https://github.com/ybouane/liquidglass) (WebGL refraction). `src/os/Glass.js` re-creates the instance when windows open or close, tells it which pixels changed each wallpaper frame instead of flagging the canvas dynamic (which would re-rasterize every layer every frame), and skips or throttles the expensive content capture of windows nothing overlaps. If the frame rate stays under 24 fps the windows fall back to the CSS material (Settings › Glass on windows); Effects off disables WebGL glass entirely.
